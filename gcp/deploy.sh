...
gcloud functions deploy "${FUNCTION_NAME}" \
    ...
    --memory=256MB \
    --timeout=60s \
    --set-env-vars="VIDEOS_BUCKET=${BUCKET_NAME},GCP_PROJECT=${PROJECT_ID},FIREBASE_API_KEY=${FIREBASE_API_KEY},REQUIRE_EMAIL=${REQUIRE_EMAIL},REQUIRE_PHONE=${REQUIRE_PHONE},ENABLE_SHARE_URL=${ENABLE_SHARE_URL},ENABLE_LABEL_SHARING=${ENABLE_LABEL_SHARING},SHARE_UPLOAD_URL_EXPIRY_HOURS=${SHARE_UPLOAD_URL_EXPIRY_HOURS},SHARE_DOWNLOAD_URL_EXPIRY_HOURS=${SHARE_DOWNLOAD_URL_EXPIRY_HOURS},APP_DISPLAY_NAME=${APP_DISPLAY_NAME}" \
    --project="${PROJECT_ID}" \
    --quiet
